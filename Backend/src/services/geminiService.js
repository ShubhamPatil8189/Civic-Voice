...
exports.analyzeIntent = async (text, history = [], directMatches = [], isLongConversation = false, language = "en") => {
  ...
  const prompt = `
    ...
    - ALL fields (explanation, follow_up_question, etc.) must be in ${targetLanguage}.
    - Scheme names can remain in English, but descriptions and explanations MUST be in ${targetLanguage}.
    - If the target language is Hindi, write in Devanagari script (हिंदी में लिखें).
    - If the target language is Marathi, write in Devanagari script (मराठीत लिहा).
    ...
    - If information is missing, ask a ONE-SENTENCE follow-up question IN ${targetLanguage}.
    ...
    - IF the user's intent is complex or broad (e.g. "I want a loan", "Help me with farming") and you haven't confirmed it yet:
      ...
      - SET 'follow_up_question' to: "Let me confirm: you are asking about [Topic] in [State]. Is that correct?" IN ${targetLanguage}
    ...
    ${isLongConversation ? `- This conversation is getting long. You MUST append this exact phrase to the end of your "explanation" field IN ${targetLanguage}: "Would you like a quick summary of what we discussed so far?" (Do NOT put this in follow_up_question).` : ''}
    ...
    - IF you are unsure (< 60% confidence) OR the user asks something out of scope (e.g., non-civic topics, illegal acts):
      1. SUMMARIZE what you *do* understand (e.g., "I understand you are asking about...") IN ${targetLanguage}.
      2. STATE CLEARLY that you cannot provide a definitive answer IN ${targetLanguage}.
      3. REDIRECT to official channels: "For the most accurate guidance, please visit your local Tehsil office or call the toll-free helpline 1905." IN ${targetLanguage}
      4. SET 'confidence' to a low value (e.g., 0.5).

    - Provide a helpful, natural language explanation (max 2-3 sentences) that directly answers the user IN ${targetLanguage}.
    ...
    Return ONLY a JSON object with these fields:
    {
      ...
      "follow_up_question": "string IN ${targetLanguage}",
      "explanation": "string IN ${targetLanguage}",
