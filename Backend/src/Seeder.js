...
// Load data
const stepsData = JSON.parse(fs.readFileSync("./steps.json", "utf-8"));
...
async function updateSchemes() {
    try {
        ...
        for (const stepData of stepsData) {
            const { schemeId, steps } = stepData;
            await Step.updateOne(
                { schemeId },
                { $set: { steps } },
                { upsert: true }
            );
            console.log(`Updated steps for scheme: ${schemeId}`);
        }

        console.log("All schemes and steps updated successfully!");
        mongoose.connection.close();
    } catch (err) {
        console.error("Error updating schemes/steps:", err);
    }
}
