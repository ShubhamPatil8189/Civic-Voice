...
const generateSmartFAQs = async (req, res) => {
    try {
      ...
      `;
            ...
            // Use the SDK for cleaner implementation if possible, or fix the fetch call
            // We'll stick to the fetch call but fix the API Key
            const API_KEY = process.env.GEMINI_API_KEY; // Fixed: Matches .env variable

            if (!API_KEY) throw new Error("GEMINI_API_KEY is missing in environment variables.");

            // Use SDK for better handling
            ...
            // Handle cases where Gemini wraps JSON in markdown or adds extra text
